...
export default async function BlogDetailPage({ params }: PageProps) {
  ...
  return (
    <div className='mt-16 border-2 p-8'>
      ...
      {/* ブログページへ戻れるリンク */}
      {/* ページを移動するナビゲーションにはソフトとハード2種類存在する。
      キャッシュを利用するナビゲーションがソフトであり、利用しないナビゲーションがハードとなる。
      キャッシュを利用しても良いページに遷移するのであればソフトナビゲーションを利用する
      */}
      <Link href={`/blogs`}>
...
/**
 * ビルド時にSupabaseからブログIDを全て取得する関数オブジェクト
 */
export async function generateStaticParams() {
  ...
  // 返り値であるResponseインターフェースはそのままでは本文を取得できない。するには.json()や.text()メソッド等を使用する必要がある
  const blogs: Blog[] = await res.json()
